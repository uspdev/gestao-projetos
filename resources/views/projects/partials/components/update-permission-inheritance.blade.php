@can('update', $project)
  <div class="dropdown">
    <button class="btn btn-sm btn-outline-secondary" type="button"
      id="project-permission-inheritance-dropdown-{{ $project->id }}"
      data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"
      title="Alterar heranca de permissoes">
    ...
    <div class="dropdown-menu dropdown-menu-right p-2" aria-labelledby="project-permission-inheritance-dropdown-{{ $project->id }}">
      @foreach (\App\Enums\Project\ProjectPermissionInheritance::cases() as $permissionInheritance)
        <form method="POST" action="{{ route('projects.updatePermissionInheritance', $project) }}" class="mb-1">
          ...
          <button type="submit" class="btn btn-sm btn-block text-left"
            @disabled($project->permission_inheritance?->value === $permissionInheritance->value)>
            <span class="badge badge-light border">{{ $permissionInheritance->label() }}</span>
