...
<x-card.preview class="mb-3 shadow-sm border-left-primary" href="{{ route('projects.show', $project) }}"
  ...
  :status-class="'badge-' . $project->status->color()" :subproject-label="$project->isSubproject() ? 'Subprojeto' : null" subproject-class="badge-light border text-muted" :project-name="$project->isSubproject() ? $project->parent?->name : null"
  :project-type="$projectType" :role-label="$userRole?->label() ?? 'Sem vínculo'" :role-class="'badge-' . ($userRole?->color() ?? 'light border text-muted')" :tags="$allTags" :tags-limit="2">
  @if ($canPin)
    <x-slot:action>
