<div class="card-header d-flex flex-column flex-sm-row align-items-center py-2">
  ...
  <div class="d-flex flex-nowrap align-items-center">
    @can('create', [\App\Models\Media::class, $owner])
      <form action="{{ $uploadRoute }}" method="post" enctype="multipart/form-data" class="mb-0 mr-2"
        ...
        <label class="btn btn-sm btn-primary mb-0 text-nowrap"
    ...
    @can('create', [\App\Models\Link::class, $owner])
      <button type="button" class="btn btn-sm btn-outline-secondary mb-0 mr-2 text-nowrap" data-toggle="modal"
    ...
    @if ($owner instanceof \App\Models\Meeting && ($shareableFileGroups->isNotEmpty() || $shareableLinkGroups->isNotEmpty()))
      <button type="button" class="btn btn-sm btn-outline-secondary mb-0 text-nowrap" data-toggle="modal"
        data-target="#{{ $componentId }}-share-links-modal"><i class="fas fa-share-alt mr-1"
          aria-hidden="true"></i>Compartilhar com reunião</button>
    @endif
  </div>
</div>
