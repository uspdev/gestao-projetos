...
- [ ] O Projeto é owner direto das chaves sob o alias `project`; o gerenciador da biblioteca aparece em um único card das configurações e permite emitir, renovar e revogar credenciais sem recriar essas operações na aplicação.
...
- [ ] `GET /api/projects/{project}` exige `projects.read` e escopo idêntico ao Projeto proprietário; resposta inclui descrição integral, estado, tipo, fase, Projeto pai, tags, `modules.enabled`, URL web e timestamps, sem membros, coleções embutidas ou `modules.tasks_enabled`.
