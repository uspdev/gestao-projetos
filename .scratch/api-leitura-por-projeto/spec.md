...
20. Como consumidor da API, quero que o detalhe do Projeto contenha todas as informações disponibilizadas pela interface a um visualizador, para obter seu contexto completo em uma requisição.
...
43. Como responsável pela privacidade, quero identificar membros, responsáveis e autores por ID e nome, sem expor e-mail ou outros dados pessoais desnecessários.
...
- A biblioteca `uspdev/api-keys` e o requisito PHP `^8.3` já estão presentes. O Projeto será o Projeto proprietário direto sob o alias `project`, resolverá as abilities e usará o gerenciador da biblioteca. Não haverá tabela intermediária de Sistemas clientes.
...
- A exclusão lógica do Projeto revogará imediatamente suas chaves ativas. Restauração não reativará credenciais antigas. A leitura do Projeto retornará todo o conteúdo visível a um visualizador: dados gerais, visibilidade, herança, tipo, fase, Projeto pai, tags, módulos e seus estados, membros e papéis, comentários, Arquivos, Links, Menções recebidas, subprojetos e Reuniões em cuja Pauta aparece. Não haverá `modules.tasks_enabled`.
- Reuniões exigem vínculo direto com o Projeto da URL, inclusive quando ele é subprojeto. Uma Reunião compartilhada entre Projetos será devolvida integralmente, com Pauta e referências externas resumidas, sem conceder acesso aos detalhes dos recursos externos. A lista retorna ID, título, estado, data agendada, local, Projetos vinculados, timestamps e URL web; o detalhe acrescenta Anotações prévias, Ata, Transcrição, Pauta com ID e posição, comentários com IDs, Arquivos e Links próprios ou compartilhados e Menções recebidas.
...
- A lista de Tarefas manterá a representação atual, incluindo descrição integral, estado e prioridade, datas, tags, URL web e responsáveis pelo nome. O detalhe acrescentará Projeto, responsáveis com ID e papel, comentários, Arquivos e Links próprios e Menções recebidas. A lista aceitará `status[]`, `priority[]`, `due_from`, `due_to`, `tag[]` por slug com correspondência a qualquer tag e `search` em título/descrição. Datas de vencimento são inclusivas em `YYYY-MM-DD`; a ordem fixa é atualização e ID decrescentes; Tarefas concluídas continuam incluídas. Com módulo desativado, lista e detalhe responderão `409 tasks_module_disabled`.
...
- Endpoint próprio de Links externos, mistura de Links com Arquivos, miniaturas na API e detalhe JSON separado de Arquivo.
