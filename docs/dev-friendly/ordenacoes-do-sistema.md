...
| Navegador de Arquivos da reunião | Arquivos próprios da página atual vêm antes dos compartilhados; duplicatas mantêm a primeira ocorrência. A interface separa o resultado, preservando a sequência, em Imagens e depois Arquivos nas abas. | A lista própria possui o desempate por `id`; a compartilhada não. A separação por tipo não reordena os itens dentro de cada aba. | [`components/files/list.blade.php`](../../resources/views/components/files/list.blade.php#L10), [`MeetingController::show`](../../app/Http/Controllers/MeetingController.php#L132) |
