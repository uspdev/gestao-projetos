...
O código JavaScript permanecerá em `resources/js` como fonte canônica e será publicado em `public/js` pelo comando `php artisan projetos:assets-publish`, executado automaticamente pelo evento `post-autoload-dump` do Composer. O comando apenas copia essa árvore, sem empacotar, concatenar, reescrever ou versionar os arquivos. Os estilos próprios específicos de componentes permanecerão nos blades e serão adicionados às páginas por `@pushOnce('styles')`; não haverá folhas CSS próprias para publicar em `public/`.
