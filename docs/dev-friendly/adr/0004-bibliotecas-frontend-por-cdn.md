...
O código JavaScript e os estilos próprios permanecerão em `resources/` como fontes canônicas e serão publicados em `public/` pelo comando `php artisan projetos:assets-publish`, executado automaticamente pelo evento `post-autoload-dump` do Composer. O comando apenas copia as árvores `resources/js` e `resources/css`, sem empacotar, concatenar, reescrever ou versionar os arquivos. O layout usa `asset()` para carregá-los: o JavaScript adota módulos ES nativos com `type="module"`, e cada folha CSS é vinculada diretamente, sem `@import`. Os arquivos copiados em `public/` continuarão fora do controle de versão.

**Revisão de 2026-08-11:** esta decisão substitui o publicador anterior, que simulava CommonJS em PHP, incorporava importações CSS e gerava `mix-manifest.json`. Node.js, npm, Laravel Mix e Webpack não participam da publicação padrão dos ativos. A revisão preserva a restrição de produção sem Node.js e elimina uma etapa de compilação específica da aplicação.
