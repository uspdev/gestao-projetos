...
recurso. Os endpoints de detalhe espelharão todas as informações da entidade
que a interface disponibiliza a um usuário com permissão de visualização.
Ações, capacidades de mutação e preferências pessoais do leitor não integram
essa equivalência.
...
pertence ou está vinculado ao Projeto da URL e que esse Projeto é o Projeto
proprietário da Chave de API autenticada.
...
Ata e Transcrição integrais, a Pauta ordenada com identificador e posição, os
comentários ativos com identificadores do comentário e do autor, Arquivos e
Links próprios ou compartilhados e Menções recebidas. Os metadados dos
Arquivos serão incorporados, mas os bytes continuarão disponíveis somente no
endpoint de download. A coleção não repetirá os registros textuais longos,
especialmente a Transcrição, cujo limite atual é 100.000 caracteres.
...
Links visíveis na interface integrarão os detalhes de Projeto, Tarefa e
Reunião, separados entre `owned` e `shared` quando houver compartilhamento.
Eles não serão misturados em `/files` nem receberão endpoints próprios nesta
etapa.

Arquivos e Links incorporados usarão os grupos `owned` e `shared`, sem um
campo genérico de acesso. Projeto e Tarefa terão o grupo `shared` vazio porque
o compartilhamento atual tem Reuniões como destino. Menções recebidas serão
agrupadas pela entidade de origem e só serão devolvidas quando a origem também
estiver no escopo do Projeto proprietário da Chave; uma Menção nunca ampliará
esse escopo.
