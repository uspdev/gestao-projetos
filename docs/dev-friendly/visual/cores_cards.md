...
O dourado suave `#D5A13A` é usado como divisor inferior compacto no cabeçalho do módulo de Reuniões e nos cards de Configurações. O card principal do Projeto não recebe divisor; seus cards internos recebem o divisor azul de Projeto.
...
- Aplicar divisores inferiores de `1px` aos cabeçalhos de conteúdo, informações, recursos, configurações e aos cards contextuais de Projeto, Tarefa e Reunião.
...
- Remover os fundos coloridos e as superfícies da dashboard; as classes de configuração servem apenas para aplicar o divisor inferior.
